...
<div class='col-sm-9' >
	...
	<div class="row" id="SearchedDataAppear"> <!-- This row is for displaying search result -->
		<?php 
		$sql1 = "SELECT * from testing WHERE (ItemCategory LIKE '%$id%' AND ItemStatus = 'Available') OR (MeetingPoint LIKE '%$id%' AND ItemStatus = 'Available') LIMIT $page1,8";
		$request = $connection-> query($sql1);
			if ($request-> num_rows > 0 ){
				while ($row=$request->fetch_assoc()){
					...
					echo "<div id='upper' style='height: 20px;'>";
					echo "<div id='left'>";
					echo "<a href='http://localhost/finalFYP/fitemDetail/itemDetail.php?productid=".$row['ID']."'> <p>  &emsp;" . $row['ItemName'] ."</a>";
					echo "<br> &emsp; <img id='condition' alt='Not found' src='tick.png' title='Condition'>  " .$row['Conditions'] ;
					echo "<br> &emsp; <img id='condition' alt='Not found' src='location.png' title='Location'>  " .$row['MeetingPoint']."</p>" ;
					echo "</div>";
					echo "<div id='right'>";
					...
					echo "<hr>";	
						$PostID = $row['ID'];
						$sqlSelectImage= "SELECT * from itempicture WHERE PostID = '$PostID' LIMIT 1";
						$result = $connection-> query($sqlSelectImage);
						if ($result -> num_rows>0){
							while ($row= $result->fetch_assoc()){
							echo "<img id='thumbnil' alt='Not found' src='http://localhost/finalFYP/images/".$row['Picture1']. "' >";
							}
						}
					echo "</div>";
					echo "</div>";
...
<?php
$sqlCheckNumRow = "SELECT * from testing WHERE ItemCategory LIKE '%$id%' AND ItemStatus = 'Available'";
...
<div class="row">
	...
	?>
		<a href="successSearch.php?id=<?php echo $id;?>&page=<?php echo $c;?>"> < </a>
	<?php 
		}
	?>
	<?php
		for ($b=1; $b<=$a; $b++){
	?> 
	<a href="successSearch.php?id=<?php echo $id;?>&page=<?php echo $b;?>" style="text-decoration:none"><?php echo $b."";?></a><?php }?>
	...
	<div class="row">
	<div class='col-sm-12' >
	<?php echo file_get_contents("http://localhost/finalFYP/bmainpage/footer.php");?>
	</div>
	</div>
